    ...
    @if($productos->count() > 0)
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @foreach($productos as $producto)
                {{-- Parte de la tarjeta de producto en home.blade.php - SIN botón añadir al carrito --}}
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                ...
                <div class="p-4">
                    <div class="flex justify-between items-start mb-2">
                        <h3 class="font-semibold text-lg text-gray-800 line-clamp-2">{{ $producto->nombre }}</h3>
                        <span class="text-lg font-bold text-green-600">{{ $producto->precio }}€/{{ $producto->unidad_medida_texto }}</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between text-sm text-gray-500 mb-4">
                        ...
                        <span class="flex items-center">
                            ...
                            {{ $producto->cantidad_inventario }} disponibles
                        </span>
                    </div>
                    
                    {{-- Solo botón de ver producto - SIN añadir al carrito --}}
...
document.addEventListener('DOMContentLoaded', function() {
    // Gestión simplificada del carrito - solo para mostrar contador
    const carrito = {
        items: [],
    ...
    // Actualizar contador cada vez que se enfoque la ventana (por si se añadió algo desde otra pestaña)
    window.addEventListener('focus', function() {
        carrito.loadFromStorage();
        carrito.updateCounter();
    });
